...
class App:
    ...
    def _sec3_compresor(self):
        ...
        mk_lbl(r4, "  FS se aplica a cada caudal individual · q_comp = Σ q_FAD_ef_i (FS incluido)",
    ...
    def _sec4_deposito(self):
        ...
        mk_lbl(body,
               "Fórmula:  V = (t · Q_peak · P_atm) / (P_max − P_min)",
               C["accent3"], ("Consolas", 10, "bold")).pack(anchor="w", pady=(0,6))
        mk_lbl(body,
               "Q_peak = q_FAD máx. instantáneo con FS  |  P_atm de sección 1  |  "
               "P_min = presión de trabajo del compresor  |  t[s]  Q[m³/s]  P[bar abs]  →  V[m³]",
               C["text3"], ("Segoe UI", 9)).pack(anchor="w", pady=(0,10))

        r1 = tk.Frame(body, bg=C["panel"])
        r1.pack(fill="x", pady=(0,6))
        mk_lbl(r1, "Tiempo de autonomía  t  (s):").pack(side="left", padx=(0,8))
        self.var_dep_t = tk.StringVar()
        mk_entry(r1, self.var_dep_t, 10).pack(side="left", padx=(0,48))
        mk_lbl(r1, "Presión máxima acumulador  P_max  (bar g):").pack(side="left", padx=(0,8))
        self.var_dep_pmax = tk.StringVar()
        mk_entry(r1, self.var_dep_pmax, 10).pack(side="left")

        mk_sep(body)
        tk.Button(body, text="✔  Confirmar configuración del acumulador",
                  bg=C["ok"], fg=C["bg"],
                  activebackground="#00a040", activeforeground=C["bg"],
                  relief="flat", bd=0, padx=14, pady=6, cursor="hand2",
                  font=("Consolas", 9, "bold"),
    ...
    def _confirmar_sec4(self):
        t_s    = self.var_dep_t.get().strip()
        pmax_s = self.var_dep_pmax.get().strip()
        if not t_s or not pmax_s:
            self._sec4.set_estado("error", "FALTAN DATOS")
            return
        try:
            parse(t_s); parse(pmax_s)
        except ValueError:
            self._sec4.set_estado("error", "VALORES INVÁLIDOS")
            return
        self._sec4.set_estado("ok", f"t={fmt(parse(t_s),1)}s  P_max={fmt(parse(pmax_s),2)} bar g")
        self._sec4.cerrar()
    ...
    def _sec5_resultados(self):
        ...
        # Total Q_FAD máx instantáneo
        tot_max_f = tk.Frame(left, bg=C["card"],
                             highlightthickness=1,
                             highlightbackground=C["border2"])
        tot_max_f.pack(fill="x", pady=(0,8))
        tk.Label(tot_max_f, text="TOTAL  q_FAD máx. instantáneo  (sin % uso)",
                 bg=C["card"], fg=C["text2"],
                 font=("Consolas", 9, "bold")).pack(side="left", padx=12, pady=6)
        self._var_total_max = tk.StringVar(value="—")
        self.lbl_total_max = tk.Entry(tot_max_f, textvariable=self._var_total_max, width=22,
                                      bg=C["card"], readonlybackground=C["card"],
                                      fg=C["accent2"], font=("Consolas", 11, "bold"),
                                      relief="flat", bd=0, highlightthickness=0,
                                      state="readonly")
        self.lbl_total_max.pack(side="right", padx=12)
        ...
        self._rd = {}
        for key, lbl_txt, col in [
            ("vol",    "Volumen acumulador (m³):",        C["accent3"]),
            ("q_peak", "Q_peak c/FS (m³/min):",          C["text2"]),
            ("t_dep",  "Tiempo autonomía t (s):",         C["text2"]),
            ("p_max",  "P_max acumulador (bar g):",       C["text2"]),
            ("p_min",  "P_min = P_trab compresor (bar g):", C["text2"]),
        ]:
    ...
    def _guardar(self):
        ...
        data = {
            ...
            "fs":         self.var_fs.get(),
            "dep_t":      self.var_dep_t.get(),
            "dep_pmax":   self.var_dep_pmax.get(),
    ...
    def _cargar(self):
        ...
        self.var_dep_t.set(data.get("dep_t", ""))
        self.var_dep_pmax.set(data.get("dep_pmax", ""))
    ...
    def _calcular(self):
        try:
            ...
            for d in datos:
                ...
                # FS aplicado a nivel individual
                qFAD_max_fs = qFAD * fs_factor                    # máx instantáneo con FS
                qFAD_ef     = qFAD * (d["uso"] / 100.0) * fs_factor  # efectivo con FS
                total_FAD     += qFAD_ef
                total_FAD_max += qFAD_max_fs
                if d["p_req"] > max_p:
                    max_p = d["p_req"]
                filas_r.append((d["nombre"], d["cant"], qN, qFAD, d["uso"], qFAD_ef, qFAD_max_fs))
            ...
            q_comp = total_FAD  # los individuales ya llevan FS incorporado

            # Depósito — fórmula: V = (t · Q_peak · P_atm) / (P_max − P_min)
            t_dep_s    = self.var_dep_t.get().strip()
            pmax_dep_s = self.var_dep_pmax.get().strip()
            if not t_dep_s or not pmax_dep_s:
                messagebox.showerror("Acumulador",
                                     "Complete el tiempo t y P_max en la sección 4.")
                return
            t_dep    = parse(t_dep_s)           # segundos
            pmax_dep = parse(pmax_dep_s)        # bar g

            # Convertir unidades para la fórmula
            # Q_peak en m³/s  (total_FAD_max está en m³/min)
            q_peak_s = total_FAD_max / 60.0
            # P en bar abs
            p_atm_abs  = p_atm                  # ya está en bar abs (sección 1)
            p_max_abs  = pmax_dep + p_atm        # bar g → bar abs
            p_min_abs  = p_trab   + p_atm        # presión de trabajo del compresor → bar abs

            if p_max_abs <= p_min_abs:
                messagebox.showerror("Error acumulador",
                    f"P_max ({fmt(pmax_dep,2)} bar g = {fmt(p_max_abs,3)} bar abs) debe ser "
                    f"mayor que P_min = presión de trabajo ({fmt(p_trab,3)} bar g = {fmt(p_min_abs,3)} bar abs).")
                return

            tamano = (t_dep * q_peak_s * p_atm_abs) / (p_max_abs - p_min_abs)

            # ── Tabla de resultados ──
            for w in self.frame_res.winfo_children():
                w.destroy()

            for nombre, cant, qN, qFAD, uso, qFAD_ef, qFAD_max_fs in filas_r:
                rf = tk.Frame(self.frame_res, bg=C["panel"])
                rf.pack(fill="x", pady=1)
                for txt, cw, col in [
                    (f"{nombre} ×{cant}", 22, C["text"]),
                    (fmt(qN, 1),           14, C["text2"]),
                    (fmt(qFAD_max_fs, 4),  20, C["accent2"]),
                    (f"{uso:.0f}%",         7, C["text2"]),
            ...
            self._var_total_max.set(f"{fmt(total_FAD_max,4)} m³/min")
            ...
            self._rd["q_peak"]._var.set(f"{fmt(total_FAD_max,4)} m³/min")
            self._rd["t_dep" ]._var.set(f"{fmt(t_dep,1)} s")
            self._rd["p_max" ]._var.set(f"{fmt(pmax_dep,2)} bar g")
            self._rd["p_min" ]._var.set(f"{fmt(p_trab,3)} bar g")
            v_str = fmt(tamano, 3)

            # Guardar todos los resultados calculados para Excel
            self._last_results = dict(
                t_amb=t_amb, p_atm=p_atm,
                total_FAD=total_FAD, total_FAD_max=total_FAD_max, q_comp=q_comp,
                p_base=p_base, dp_can=dp_can, dp_sala=dp_sala,
                p_trab=p_trab, fs=fs, modo=modo,
                t_dep=t_dep, pmax_dep=pmax_dep, p_min_abs=p_min_abs,
                p_max_abs=p_max_abs, tamano=tamano,
            )
            ...
            self.lbl_nota.config(
                text=(
                    ...
                    f"FS={fmt(fs,1)}% aplicado a cada caudal individual  |  "
                    f"q_comp = Σ q_FAD_ef_i (FS ya incluido)  |  "
    ...
    def _exportar_excel(self):
        ...
        hdrs2 = ["Equipo","Cant.","P req (bar g)","Caudal","Tipo caudal",
                 ...
                 "q_N (Nl/min)","q_FAD máx inst. (m³/min)","q_FAD efectivo (m³/min)"]
        ...
        for fila in self.filas:
            d = fila.to_dict()
            # Find matching result row (7-element tuple)
            match = next(((n,ca,qn,qf,us,qfe,qfm) for (n,ca,qn,qf,us,qfe,qfm) in fr
            ...
            vals = [
                ...
                round(match[6],4) if match else "",   # q_FAD máx inst. con FS
                round(match[5],4) if match else "",   # q_FAD efectivo con FS
            ]
        ...
        row += 1
        # Total Q_FAD máx instantáneo
        ws.cell(row=row, column=9,  value="TOTAL q_FAD máx. instantáneo (sin % uso)").font = Font(bold=True, name="Consolas")
        c2 = ws.cell(row=row, column=11, value=round(r["total_FAD_max"],4))
        c2.font = Font(bold=True, name="Consolas", color="00E5A0")
        row += 2
        ...
        # ── Acumulador ──
        sec_title(ws, row, "4. DEPÓSITO / ACUMULADOR  —  V = (t · Q_peak · P_atm) / (P_max − P_min)"); row += 1
        tam = r["tamano"]
        for label, val in [
            ("Volumen acumulador (m³)",               round(tam, 4)),
            ("Q_peak con FS (m³/min)",                round(r["total_FAD_max"], 4)),
            ("Tiempo autonomía t (s)",                 round(r["t_dep"], 1)),
            ("P_atm (bar abs)",                        round(r["p_atm"], 5)),
            ("P_max acumulador (bar g)",               round(r["pmax_dep"], 2)),
            ("P_max acumulador (bar abs)",             round(r["p_max_abs"], 3)),
            ("P_min = P_trab compresor (bar g)",       round(r["p_trab"], 3)),
            ("P_min (bar abs)",                        round(r["p_min_abs"], 3)),
        ]:
            ws.cell(row=row, column=1, value=label).font = Font(name="Consolas")
            c = ws.cell(row=row, column=3, value=val)
            val_style(c); row += 1
